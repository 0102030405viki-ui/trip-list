...
function PhotoAlbum({ photos, onAddPhotos }) {
  ...
  return <motion.div className="album" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
    ...
    {photos.length ? <div className="photo-grid">{photos.map(photo => <motion.button key={photo.id} className="photo-tile" whileHover={{ scale: 1.025 }} onClick={() => setSelected(photo)}><img src={photo.src} alt={photo.name || "Trip memory"} /></motion.button>)}</div> : <div className="album-empty"><Camera size={25} /><p>Your memories will live here.</p><span>Add your first photos from this trip.</span></div>}
    <AnimatePresence>{selected && <motion.div className="photo-lightbox" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelected(null)}><button className="icon-button" onClick={() => setSelected(null)}><X size={20} /></button><img src={selected.src} alt={selected.name || "Trip memory"} onClick={e => e.stopPropagation()} /></motion.div>}</AnimatePresence>
  </motion.div>;
}
